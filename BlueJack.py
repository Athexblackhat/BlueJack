...
def setup_bluetooth(target_address, adapter_id):
    ...
    adapter.set_property("name", "BlueJack")
...
# Main function
def main():
    parser = argparse.ArgumentParser(description="BlueJack - Bluetooth HID Attack Tool")
    ...
    if not os.path.exists(payload_folder):
        ...
        log.info(f"Created payloads folder at {payload_folder}")
        log.info("Please add your DuckyScript payloads to the payloads folder and run again.")
        return
    
    payloads = [f for f in os.listdir(payload_folder) if f.endswith('.txt')]

    if not payloads:
        log.info("No .txt payload files found in payloads folder. Exiting.")
        return
    ...
    if not duckyscript:
        log.info("Payload file is empty or not found. Exiting.")
        return
